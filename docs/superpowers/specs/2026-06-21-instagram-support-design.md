...
## 十七、实现说明补充（2026-06-21）

首版实现与本设计基本一致，以下为落地时的具体取舍与补充：

- collector（`scripts/instagram-collector.mjs`）默认以 Playwright headless 模式运行。
- collector 通过 `HTTPS_PROXY` / `HTTP_PROXY` 环境变量读取代理；Rust 命令侧会把检测到的系统代理注入子进程环境，复用与 yt-dlp 一致的代理来源。
- 登录态桥接：注入 `sessionid` 后会先访问一次 `instagram.com` 预热，再导出 BrowserContext 内的完整 cookies 为临时 `cookies.txt`，作为采集与下载共用的登录态。
- yt-dlp 的 Cookie 选择逻辑调整为「显式 `cookies.txt` 路径优先于浏览器 Cookie 来源」；Instagram 采集即使在「Chrome」来源下，也会通过桥接文件路径生效。
- 「详情页连续下一条」基于 `aria-label="Next"` 控件与 URL 变化判断推进，并对重复 URL 去重，遇到无法继续时提前结束并给出数量不足的 warning。
- Story 仅做实验性支持：当前实现只尽力采集入口可见的当前 Story，失败时作为实验性能力单独提示，不影响其他模式。
- 前端将 Instagram 配置收敛为独立的「Instagram 访问」设置块（sessionid / cookies.txt 路径 / 采集模式 / 抓取数量），未改动通用 Cookie 设置。
