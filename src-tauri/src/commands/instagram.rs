...
fn resolve_collector_script() -> PathBuf {
    ...
    log::info!("[instagram] resolved collector script: {}", resolved.display());
    resolved
}
...
fn run_collector(
    ...
    log::info!(
        "[instagram] run_collector: url={} mode={:?} count={} has_sessionid={} cookie_file_path={:?}",
        request.url,
        request.mode,
        request.count,
        request.sessionid.is_some(),
        request.cookie_file_path
    );
    ...
    if let Some(proxy) = crate::platform::proxy::detect_proxy() {
        log::info!("[instagram] proxy detected: {proxy}");
        command.env("HTTPS_PROXY", &proxy);
        command.env("HTTP_PROXY", &proxy);
    } else {
        log::info!("[instagram] no system proxy detected");
    }

    let mut child = command
        .spawn()
        .map_err(|error| {
            log::error!("[instagram] failed to spawn collector: {error}");
            format!("启动 Instagram 采集脚本失败：{error}")
        })?;
    ...
    let output = child
        .wait_with_output()
        .map_err(|error| {
            log::error!("[instagram] failed waiting for collector: {error}");
            format!("等待 Instagram 采集脚本结束失败：{error}")
        })?;

    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr).trim().to_string();
        log::error!(
            "[instagram] collector exited with code {:?}, stderr: {stderr}",
            output.status.code()
        );
    ...
    log::info!(
        "[instagram] collector stdout length={} first_200={:?}",
        stdout.len(),
        &stdout[..stdout.len().min(200)]
    );

    let response = parse_collector_output(&stdout)?;
    log::info!(
        "[instagram] parsed: resolved_count={} items={} warnings={:?} cookie_bridge_file_path={:?}",
        response.resolved_count,
        response.items.len(),
        response.warnings,
        response.cookie_bridge_file_path
    );
    Ok(response)
}
...
pub async fn collect_instagram_targets(
    ...
    log::info!(
        "[instagram] collect_instagram_targets: url={} mode={:?} count={}",
        request.url,
        request.mode,
        request.count
    );
    validate_collect_request(&request)?;
    let result = tauri::async_runtime::spawn_blocking(move || run_collector(&request))
        .await
        .map_err(|error| format!("Instagram 采集任务执行失败：{error}"))?;
    match &result {
        Ok(response) => log::info!(
            "[instagram] collect_instagram_targets succeeded: resolved_count={}",
            response.resolved_count
        ),
        Err(error) => log::error!("[instagram] collect_instagram_targets failed: {error}"),
    }
    result
}
